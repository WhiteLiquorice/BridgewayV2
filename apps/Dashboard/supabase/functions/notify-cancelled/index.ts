// Called when an appointment is cancelled by staff or by the patient
// Request body: { appointment_id: string, cancelled_by: 'staff' | 'patient' }
...
Deno.serve(async (req) => {
  ...
  try {
    const { appointment_id, cancelled_by } = await req.json() as {
      appointment_id: string
      cancelled_by: 'staff' | 'patient'
    }
    ...
    const rescheduleNote = cancelled_by === 'staff'
      ? ' Please contact us to reschedule.'
      : ''

    const smsBody = `Hi ${clientName}, your ${serviceName} appointment on ${dateStr} at ${timeStr} has been cancelled.${rescheduleNote} Reply STOP to opt out.`
    const emailSubject = `Appointment Cancelled — ${serviceName} on ${dateStr}`
    const emailHtml = `
      <h2>Appointment Cancelled</h2>
      <p>Hi ${clientName},</p>
      <p>Your <strong>${serviceName}</strong> appointment at <strong>${orgName}</strong> has been cancelled.</p>
      <table>
      ...
      ${cancelled_by === 'staff' ? `<p>Please contact us to reschedule your appointment.</p>` : ''}
    ...
    if ((notifSettings?.sms_enabled ?? true) && (notifSettings?.sms_cancelled ?? true) && appt.clients?.phone) {
      try {
    ...
    if ((notifSettings?.email_enabled ?? true) && (notifSettings?.email_cancelled ?? true) && appt.clients?.email) {
      try {
